...
class Pjax {
    ...
    _preventCheck(element, evt) {
        ...
        if (Barba.Utils.getPort() !== Barba.Utils.getPort(element.port)) return false;
        ...
        if (Barba.Utils.cleanLink(href) == Barba.Utils.cleanLink(location.href)) return false;
    ...
    _initPageTransiton() {
        ...
        this.transitionStart = (e) => this._transitionStart.bind(Barba.BaseTransition)();
        this.transitionFadeOut = (e) => this._transitionFadeOut.bind(Barba.BaseTransition)();
        this.transitionFadeIn = (e) => this._transitionFadeIn.bind(Barba.BaseTransition)();
    ...
    _sendGA() {
        Barba.Dispatcher.on('initStateChange', function() {
            if (typeof window.ga === 'function' && Barba.HistoryManager.history.length >= 1) {
                window.ga('send', 'pageview', location.pathname);
            }
            if (typeof window.gtag === 'function' && Barba.HistoryManager.history.length >= 1) { 
                window.gtag('config', 'トラッキングID', {'page_path': location.pathname}); 
            }
        });
    }
    _initEvents() {
